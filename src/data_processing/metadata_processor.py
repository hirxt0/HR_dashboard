...
class MetadataProcessorRU:
    ...
    def __init__(self, embedding_model_name: str = None):
        ...
        # self.topic_clf: either sklearn model OR dict {'type':'centroid','centroids':{topic:vector}}
        # self.topic_le: either LabelEncoder instance OR dict {'classes': [...], 'map': {...}}
        self.topic_clf = None
        self.topic_le = None
    ...
    def extract_tags(self, text: str, top_n: int = 5) -> List[str]:
        ...
        try:
            ...
            # Небольшой список стоп-слов (можно расширить)
            stopwords = {
    ...
    def train_topic_classifier(self, embeddings: np.ndarray, labels: List[str], model_path: str = None, classifier: str = 'logistic', **kwargs) -> Dict:
        ...
        - classifier: 'logistic' (по умолчанию). Можно расширить.
        - kwargs: дополнительные параметры для классификатора (например, C для LogisticRegression)
