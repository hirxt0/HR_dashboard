...
class Processor:
    ...
    def embed_chunks(self, chunks: List[Dict]) -> List[Dict]:
        ...
        embs = self.emb_model.embedding(texts, batch_size=self.batch_size)
        nembs = self.emb_model.normalized_embeddings(texts, batch_size=self.batch_size)
        for i, c in enumerate(chunks):
            c["embedding"] = embs[i].tolist()
            c["n_embedding"] = nembs[i].tolist()
        return chunks
